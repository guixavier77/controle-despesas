...
const addTransactionsIntoDOM = ({ amount, name, id }) => {
  ...
  const classCSS = amount < 0 ? 'minus' : 'plus';
  const amountWithoutOperator = Math.abs(amount);
  const li = document.createElement('li');

  li.classList.add(classCSS);
  li.innerHTML = `
    ${name} <span>${operator} R$ ${amountWithoutOperator}</span><button class="delete-btn" onClick=
    "removeTransaction(${id})">x</button>
  `;
...
const getExpenses = (transactionsAmounts) =>
  Math.abs(
    transactionsAmounts
      .filter((transactionValue) => transactionValue < 0)
